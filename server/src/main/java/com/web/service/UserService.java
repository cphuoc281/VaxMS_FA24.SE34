...
logger.info("User đã được kích hoạt thành công: {}", email);
    ...
    public void guiYeuCauQuenMatKhau(String email, String url) {
        ...
        mailService.sendEmail(email, "Đặt lại mật khẩu",
            ...
            "Hãy nhấp vào liên kết bên dưới để đặt lại mật khẩu mới của bạn<br><br>" +
    ...
    public void xacNhanDatLaiMatKhau(String email, String password, String key) {
        ...
        if (key.equals(user.getRememberKey())) {
            user.setPassword(passwordEncoder.encode(password));
            user.setRememberKey(null); // Xóa mã nhớ sau khi sử dụng
            userRepository.save(user);
        } else {
